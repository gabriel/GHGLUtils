...
static inline float GHDistanceBetweenPoints(float x1, float y1, float x2, float y2) {
...
static inline Color4 Color4MakeRGBA(CGFloat red, CGFloat green, CGFloat blue, CGFloat alpha) {
	Color4 ret;
	ret.red = red / 255.0f;
	ret.green = green / 255.0f;
	ret.blue = blue / 255.0f;
	ret.alpha = alpha;
	return ret;  
}

static inline Color4 Color4Make(CGFloat inRed, CGFloat inGreen, CGFloat inBlue, CGFloat inAlpha) {
	Color4 ret;
	ret.red = inRed;
	ret.green = inGreen;
	ret.blue = inBlue;
...
static inline Color4 Color4SetAlpha(Color4 color, CGFloat alpha) {
	color.alpha = alpha;
	return color;
}

static inline void Color4Set(Color4 *color, CGFloat inRed, CGFloat inGreen, CGFloat inBlue, CGFloat inAlpha) {
	color->red = inRed;
	color->green = inGreen;
	color->blue = inBlue;
	color->alpha = inAlpha;
...
static inline Color4 Color4Interpolate(Color4 color1, Color4 color2, GLfloat percent) {
	Color4 ret;
...
static const Vector2D Vector2DZero = {0.0f, 0.0f};

static inline Vector2D Vector2DMake(GLfloat x, GLfloat y) {
	return (Vector2D){x, y};
}

static inline Vector2D Vector2DMultiply(Vector2D v, GLfloat s) {
...
static inline GLfloat Vector2DDotProduct(Vector2D v1, Vector2D v2) {
	return (GLfloat)(v1.x * v2.x + v1.y * v2.y);
}

static inline GLfloat Vector2DLength(Vector2D v) {
	return (GLfloat)sqrtf(Vector2DDotProduct(v, v));
}

static inline Vector2D Vector2DNormalize(Vector2D v) {
	return Vector2DMultiply(v, 1.0f/Vector2DLength(v));
}

static inline GLfloat Vector2DDistance(Vector2D v1, Vector2D v2) {
  // d = sqrt [(x1 - x2)^2 + (y1 - y2)^2] 
  return sqrtf(((v1.x - v2.x) * (v1.x - v2.x)) + ((v1.y - v2.y) * (v1.y - v2.y)));
}

static inline NSString *NSStringFromVector2D(Vector2D vector) {
  return [NSString stringWithFormat:@"(%0f, %0f)", vector.x, vector.y];
}

#pragma mark Line2D

// Line for y = mx + b with start and end points
typedef struct {
  GLfloat	m;
	GLfloat b;
} Line2D;
...
static inline NSString *NSStringFromLineSegment2D(LineSegment2D line) {
  return [NSString stringWithFormat:@"(%0.2f, %0.2f), (%0.2f, %0.2f), m=%0.2f, b=%0.2f", line.v1.x, line.v1.y, line.v2.x, line.v2.y, line.m, line.b];
}

#pragma mark Time

static inline BOOL TimeDeltaMax(CFTimeInterval *lastTime, CFTimeInterval delta, CFTimeInterval max) {
  if (delta >= max) {
    *lastTime = CACurrentMediaTime() - (max - delta);
    return YES;
  }
  return NO;
}  

static inline float TimeDelta(CFTimeInterval *lastTime) {
...
static inline GHGLSize GHGLSizeMake(GLfloat width, GLfloat height) {
  GHGLSize size; size.width = width; size.height = height; return size;
}
...
static inline Vector3D Vector3DMultiply(Vector3D v, GLfloat s) {
...
static inline GLfloat Vector3DDistance(Vector3D v1, Vector3D v2) {
  return sqrtf(((v1.x - v2.x) * (v1.x - v2.x)) + ((v1.y - v2.y) * (v1.y - v2.y)) + ((v1.z - v2.z) * (v1.z - v2.z)));
}

static inline NSString *NSStringFromVector3D(Vector3D vector) {
  return [NSString stringWithFormat:@"(%0f, %0f, %0f)", vector.x, vector.y, vector.z];
}

#pragma mark Point Sprite
...
static inline NSString *NSStringFromMatrix3D(Matrix3D matrix) {
