...
- (void)setupOrtho:(GHGLView *)view {
  ...
  GHGLCheckError();
	...
	glOrthof(0.0f, view.backingWidth, 0.0f, view.backingHeight, -1.0f, 1.0f);
	glMatrixMode(GL_MODELVIEW);
}	

- (void)setupFrustum:(GHGLView *)view {
	glViewport(0, 0, view.backingWidth, view.backingHeight);
	gluPerspectivef(kFieldOfView, (GLfloat)view.backingHeight/(GLfloat)view.backingWidth, kZNear, kZFar);
}

- (void)setupView:(GHGLView *)view {
  GHGLDebug(@"Setup view; viewport: (%d, %d, %d, %d)", 0, 0, view.backingWidth, view.backingHeight);
  [self setupOrtho:view];
  glLoadIdentity();
